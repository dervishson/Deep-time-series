...
اِنَّا فَتَحْنَا لَكَ فَتْحًا مُب۪ينًاۙ 
""" 
...
def load_all_models(n_models):
    ...
    for i in range(n_models):
        ...
        filename="CNN_model_" + str(i+1) + ".h5"
...
def ensemble_predictions(members,d_input,t_input,ds_input,lab_testing):
    total_mapes=[]
    total_preds=[]
    total_results=[]
    for k in range(10):
        ...
        for j in range(0,8737,24):
            t_inputs_dem=d_input[j:j+24,:,:]
            t_inputs_temp=t_input[j:j+24,:,:]
            t_inputs_days=ds_input[j:j+24,:,:]
            t_labs=lab_testing[j:j+24,0]
            yhat = np.zeros(24)
            for i in range(23):
                yhat[i]=pred_model.predict([t_inputs_dem[i,:,:].reshape(1,shape_1,shape_2),t_inputs_temp[i,:,:].reshape(1,shape_1,shape_4),t_inputs_days[i,:,:].reshape(1,shape_1,shape_3)],batch_size=b_size)
                t_inputs_dem[i+1,shape_1-1-i:shape_1,shape_2-1] = yhat[:i+1] 
        
            yhat[i+1]=pred_model.predict([t_inputs_dem[i+1,:,:].reshape(1,shape_1,shape_2),t_inputs_temp[i+1,:,:].reshape(1,shape_1,shape_4),t_inputs_days[i+1,:,:].reshape(1,shape_1,shape_3)],batch_size=b_size)
            
            yhat=(yhat*test_std_loads)+test_mean_loads
            preds.append(yhat)
            t_labs=(t_labs*test_std_loads)+test_mean_loads
            perf=np.mean(np.abs(yhat-t_labs)/t_labs)
            mape.append(perf)
...
ten_results,ten_preds=ensemble_predictions(members,testing_inputs_dem,testing_inputs_temp,testing_inputs_days,test_labs)
...
CNN_preds=np.zeros((8760,10))
CNN_hours_preds=np.zeros((8760,1))
for i in range(10):
...
for i in range(8760):
    CNN_hours_preds[i,0]=np.mean(CNN_preds[i,:])

import scipy.io
p_dict={"CNN_hours_preds": CNN_hours_preds}
scipy.io.savemat("CNN_h_preds",p_dict)           
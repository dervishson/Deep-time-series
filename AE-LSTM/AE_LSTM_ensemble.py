...
def load_all_models(n_models):
    ...
    for i in range(n_models):
        ...
        filename="AE_LSTM_model_" + str(i+1) + ".h5"
...
AE_LSTM_preds=np.zeros((8760,10))
AE_LSTM_hours_preds=np.zeros((8760,1))
for i in range(10):
...
for i in range(8760):
    AE_LSTM_hours_preds[i,0]=np.mean(AE_LSTM_preds[i,:])

import scipy.io
p_dict={"AE_LSTM_hours_preds": AE_LSTM_hours_preds}
scipy.io.savemat("AE_LSTM_h_preds",p_dict)

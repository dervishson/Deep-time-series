...
def load_all_models(n_models):
    ...
    for i in range(n_models):
        ...
        filename="simple_GRU_model_" + str(i+1) + ".h5"
...
stacked_GRU_preds=np.zeros((8760,10))
stacked_GRU_hours_preds=np.zeros((8760,1))
for i in range(10):
    y=np.asarray(ten_preds[i])
    y=y.reshape(8760)
    stacked_GRU_preds[:,i]=y

for i in range(8760):
    stacked_GRU_hours_preds[i,0]=np.mean(stacked_GRU_preds[i,:])

import scipy.io
p_dict={"stacked_GRU_hours_preds": stacked_GRU_hours_preds}
scipy.io.savemat("stacked_GRU_h_preds",p_dict)        